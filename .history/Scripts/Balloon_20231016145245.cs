...
public partial class Balloon : RigidBody2D
{
	...
	{
		Sprite2D glow = GetNode<Sprite2D>("Glow");
		...
		{
			glow.Visible = true;
			glow.GetNode<Sprite2D>("Glow").Modulate = color;
			GetNode<Sprite2D>("PowerupSprite").Visible = true;

		}
		else
		{
			GetNode<Sprite2D>("Glow").Visible = false;
			GetNode<Sprite2D>("PowerupSprite").Visible = false;
		}
	}
