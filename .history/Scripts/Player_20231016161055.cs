...
public partial class Player : RigidBody2D
{
	...
	public override void _Process(double delta)
	{
		...
		Vector2 myPos = new Vector2((float)Mathf.Lerp(GlobalPosition.X, mousePos.X, delta * (speed)), (float)Mathf.Lerp(GlobalPosition.Y, mousePos.Y, delta * (speed)));
		Position = myPos;
		lastMousePos = mousePos;
	}

	public void IncreaseSpeed(float val = 0.2f)
	{
		speed += val;
	}
